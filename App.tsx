...
import { RateDataResponse, CurrencyCode } from './types';
...
const App: React.FC = () => {
  ...
  const loadRates = useCallback(async () => {
    ...
    try {
      ...
      console.error(err);
      const msg = err.message || "Failed to fetch market data. Please ensure the API_KEY environment variable is configured.";
      setError(msg);
    } finally {
      setIsLoading(false);
    }
  ...
  useEffect(() => {
    loadRates();
    const intervalId = setInterval(loadRates, 300000); // Auto-refresh every 5 minutes
  ...
  return (
    <div className="min-h-screen bg-slate-50 py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-4xl mx-auto space-y-8">
        ...
        {error && (
          <div className="bg-rose-50 border-l-4 border-rose-500 p-5 rounded-2xl flex flex-col space-y-3 shadow-sm animate-in slide-in-from-top duration-300">
            <div className="flex items-center space-x-3">
              ...
              <div>
                <p className="text-sm text-rose-800 font-bold tracking-tight">System Notice</p>
